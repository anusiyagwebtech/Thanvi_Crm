...
if (isset($_REQUEST['submit'])) {
	...
	if ($getid != '') {
        ...
        $linkimge = $db->prepare("SELECT * FROM `renovation` WHERE `cid` = ? ");
    ...
    if ($imag) {
        if ($pimage != '') {			
            unlink("../../images/renovation/" . $pimage);
            unlink("../../images/renovation/thump/" . $pimage);
        }
        ...
        if (($extension == 'jpg') || ($extension == 'png') || ($extension == 'gif')) {
            $m = strtolower($imagec);
            $imagev = $m . "." . $extension;
            $thumppath = "../../images/renovation/";
            $thumppath1 = "../../images/renovation/thump/";
    ...
    if ($ext != '1') {
        $msg = addrenovation($content1, $image, $imagealt, $imagename, $content, $order, $status, $ip, $getid);
    } else {
    ...
    <section class="content-header">
        <h1>
            ...
            <small><?php
                ...
                ?> renovation Mgmt </small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $sitename; ?>"><i class="fa fa-dashboard"></i> Dashboard</a></li>
            <li><a href="#"><i class="fa fa-newspaper-o"></i> Services</a></li>
            <li><a href="<?php echo $sitename; ?>pages/services/renovation.htm">renovation Mgmt</a></li>
            <li class="active"><?php
                if ($_REQUEST['cid'] != '') {
                    echo 'Edit';
                } else {
        ...
        <form method="post" autocomplete="off" enctype="multipart/form-data" action="">
            <div class="box box-info">
                    ...
                    <div class="panel panel-info" id="comp_details_fields">
						...
						<!--	<div class="clearfix"><br /></div>
                            ...
                            <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Image Name<span style="color:#FF0000;"> *</span></label>                                  
                                        <input type="text" name="imagename" pattern="[a-z0-9-_]{2,255}" title="Allowed Characters (a-z0-9-_){2,255})" placeholder="Enter the Image Name" class="form-control" value="<?php echo getrenovation('image_name', $_REQUEST['cid']); ?>" required />                     
                                    </div>
                                </div>                                           
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <div class="form-group">
                                        <label>Image <span style="color:#FF0000;"> *(Recommended Size 1000 Pixels Width * 1000 Pixels Height)</span></label>
                                        <input class="form-control spinner" <?php if (getrenovation('image', $_REQUEST['cid']) == '') { ?> required="required" <?php } ?> name="image" type="file"> 
                                    </div>
                                </div>
                                <?php if (getrenovation('image', $_REQUEST['cid']) != '') { ?>
                                    <div class="col-md-6 col-sm-6 col-xs-12" id="delimage">
                                        <label> </label>
                                        <img src="<?php echo $fsitename; ?>images/renovation/<?php echo getrenovation('image', $_REQUEST['cid']); ?>" style="padding-bottom:10px;" height="100" />
                                        <button type="button" style="cursor:pointer;" class="btn btn-danger" name="del" id="del" onclick="javascript:deleteimage('<?php echo getrenovation('image', $_REQUEST['cid']); ?>', '<?php echo $_REQUEST['cid']; ?>', 'renovation', '../../images/renovation/thump', 'image', 'cid');"><i class="fa fa-close">&nbsp;Delete Image</i></button>
                                    </div>
                                <?php } ?>
                            </div>
                            <div class="clearfix"><br /></div>
								...
								<div class="col-md-6">
									<div class="form-group">
										<label>Card Heading<span style="color:#FF0000;"> *</span></label>                                  
										 <textarea id="content1" name="content1" class="form-control" ><?php echo getrenovation('content1', $_REQUEST['cid']); ?></textarea>
									</div>
								</div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Card Content<span style="color:#FF0000;"> *</span></label>                                  
                                         <textarea id="content" name="content" class="form-control" ><?php echo getrenovation('content', $_REQUEST['cid']); ?></textarea>
                                    </div>
                                </div>
							</div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" required="required " class="form-control" placeholder="Enter the Order" name="order" value="<?php echo getrenovation('order', $_REQUEST['cid']); ?>">
                                </div>
                                <div class="col-md-6">
                                    <label>Status <span style="color:#FF0000;">*</span></label>                                  
                                    <select name="status" class="form-control">
                                        <option value="1" <?php
                                        if (getrenovation('status', $_REQUEST['cid']) == '1') {
                                            echo 'selected';
                                        }
                                        ?>>Active</option>
                                        <option value="0" <?php
                                        if (getrenovation('status', $_REQUEST['cid']) == '0') {
                                            echo 'selected';
                                        }
                                        ?>>Inactive</option>
                                    </select>
                ...
                <div class="box-footer">
                    <div class="row">
                        <div class="col-md-6">
                            <a href="<?php echo $sitename; ?>services/renovation.htm">Back to Listings page</a>
                        </div>
                        <div class="col-md-6">
